...
describe('feedback helpers', () => {
  ...
  it('returns actionable semver feedback for common mistakes', () => {
    ...
    expect(prefixed.suggestions[0]?.fix).toContain('npm version 1.2.3');

    const stale = getVersionFeedback('1.2.3', config, '1.2.3');
    expect(stale.valid).toBe(false);
    expect(
      stale.suggestions.some((suggestion) => suggestion.fix?.includes('npm version 1.2.4')),
    ).toBe(true);

    expect(getVersionFeedback('1.2', config).suggestions[0]?.fix).toContain('npm version 1.2.0');
  ...
  it('flags semver leading zeros, downgrades, and suspicious jumps', () => {
    ...
    expect(older.suggestions[0]?.fix).toContain('npm version 1.2.4');
  ...
  it('returns actionable calver feedback for invalid formats and future dates', () => {
    ...
    expect(futureYear.suggestions[0]?.fix).toContain('npm version 2026.04.05');
    ...
    expect(futureMonth.suggestions[0]?.fix).toContain('npm version 2026.03.05');
    ...
    expect(futureDay.suggestions[0]?.fix).toContain('npm version 2026.03.15');
    ...
    expect(stale.valid).toBe(false);
    expect(stale.suggestions[0]?.fix).toContain('npm version 2026.03.15.0');
  ...
  it('formats calver fixes when parsed versions omit raw input', () => {
    ...
    expect(result.suggestions[0]?.fix).toContain('npm version 2026.3.1');
  });
